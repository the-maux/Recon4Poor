...
deep_and_long_recon() {
  ...
  echo -n `date +"[%H:%M:%S]"` && echo -e "(INFO) chaos + wayback found: $(cat chaos.txt | wc -l) url(s)"
...
use_recontools_individualy() {
  ...
  echo -n `date +"[%H:%M:%S]"` && echo -e "(INFO) SubDomainizer found: $(cat SubDomainizer.txt | wc -l) domain(s) in scope"

  subfinder -d $target -silent > subfinder.txt
  echo -n `date +"[%H:%M:%S]"` && echo -e "(INFO) subfinder found: $(cat subfinder.txt | wc -l) domain(s) in scope"
  cat subfinder.txt >> SubDomainizer.txt

  python3 ./tools/Sublist3r/sublist3r.py -d $target -o sublist3r.txt &> nooutput
  echo -n `date +"[%H:%M:%S]"` && echo -e "(INFO) sublist3r found: $(cat sublist3r.txt | wc -l) domain(s) in scope"
  ...
  echo -n `date +"[%H:%M:%S]"` && echo -e "(INFO) chaos found: $(cat chaos.txt | wc -l) url(s)"
...
search_jsFile_from_domain_found() {
  ...
  echo -n `date +"[%H:%M:%S]"` && echo -e "(INFO) gau + subjs found: $(cat subjs_url.txt | wc -l) url(s)"
  cat subjs_url.txt >> all_js_files_found.txt

  jsubfinder -f listOfDomains.txt  > jsubfinder.txt #TODO add in all urls.txt
  echo -n `date +"[%H:%M:%S]"` && echo -e "(INFO) jsubfinder individually found: $(cat jsubfinder.txt | wc -l) url(s)"
  ...
  echo -n `date +"[%H:%M:%S]"` && echo -e "(INFO) hakrawler individually found: $(cat hakrawler_urls.txt | wc -l) url(s)"
  ...
  then
      echo -n `date +"[%H:%M:%S]"` && echo "(WARNING) No endpoint found"
  fi
  cat all_endpoints.txt | sort -u > endpoints.txt
  echo -n `date +"[%H:%M:%S]"` && echo "(INFO) Number of endpoint found with LinkFinder: $(cat endpoints.txt | wc -l)"
}
...
regroup_found_and_filter() {
  ...
  echo -n `date +"[%H:%M:%S]"` && echo "(INFO) Before filtering duplicate/offline/useless files, we found: $((number_of_file_found)) files to analyse"
  ...
  echo -n `date +"[%H:%M:%S]"` && echo "(INFO) Result of filters: Found $((number_of_file_found)) javascripts files to analyse"
  ...
  echo -n `date +"[%H:%M:%S]"` && echo "(INFO) Result of BIG filters: Found $((number_of_file_found_post_filter)) javascripts files to analyse"
